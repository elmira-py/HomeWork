...
@pytest.mark.parametrize("currency, expected_count", [("USD", 3), ("RUB", 2), ("EUR", 0)])  # валюта отсутствует
...
def test_filter_by_currency_mixed_valid_and_invalid():
    mixed = [{"id": 1, "operationAmount": {"currency": {"code": "USD"}}}, {"id": 2}]  # валидная  # битая
...
@pytest.mark.parametrize(
    "start, stop, expected",
    [
        (11, 13, ["0000 0000 0000 0011", "0000 0000 0000 0012", "0000 0000 0000 0013"]),  # базовые случаи
        (1234, 1234, ["0000 0000 0000 1234"]),  # совпадение  start и stop
        (1201, 1202, ["0000 0000 0000 1201", "0000 0000 0000 1202"]),
        (
            9999999999999998,
            9999999999999999,  # крайние значения диапазона
            ["9999 9999 9999 9998", "9999 9999 9999 9999"],
        ),
        (5, 3, []),
    ],
)  # start > stop, должен вернуть пустой список
